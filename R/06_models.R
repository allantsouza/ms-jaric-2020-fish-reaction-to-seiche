...
tic('Model run takes')
mld_gamm_pike_day_k100  <- bam(formula =
                           det_depth ~
                           s(seasonal_depth, k = 100, bs = 'cr') +
                           s(amplitude, k = 100, bs = 'cr')+
                           s(mean_gradient,  k = 100, bs = 'cr') +
...
acf_resid(model = mld_gamm_pike_day_k100, split_pred = 'fishid')
par(mfrow = c(2,2))

# Model summary
summary.gam(mld_gamm_pike_day_k100)

# Extract R^2 
summary(mld_gamm_pike_day_k100)$r.sq   
[1] 0.745     # R^2 is not bad but can improve

# Check overdispersion (phi)
sum(resid(mld_gamm_pike_day_k100, type = "pearson")^2) / df.residual(mld_gamm_pike_day_k100)
[1] 2.380257  # too high (phi > 1.5)

# 2.1. Extract and round edf of the smooth terms (estimated with penalization) to fix them
f.df <- round(summary(mld_gamm_pike_day_k100)$edf)+1  # get edf per smooth
...
mld_gamm_pike_day_k100.fx <-  bam(formula =
                              det_depth ~
...
# 2.3. Compare models 
AIC(mld_gamm_pike_day.fx,mld_gamm_pike_day_k50, mld_gamm_pike_day_k50.fx, mld_gamm_pike_day_k100, mld_gamm_pike_day_k100.fx)
                                df      AIC
mld_gamm_pike_day.fx       2244.0729 214831.3
mld_gamm_pike_day_k50       441.6427 239347.4
mld_gamm_pike_day_k50.fx   2248.3288 213448.5    # The 50k model with fixed edf is very close to 100k
mld_gamm_pike_day_k100      521.2288 237957.6
mld_gamm_pike_day_k100.fx  2281.9432 213072.3    # The 100k model with fixed edf has the lowest AIC

# Is the 100k model with fixed edf preferred to 50k?
compareML(mld_gamm_pike_day_k50.fx, mld_gamm_pike_day_k100.fx)  # 100k fx still has lower fREML score and AIC 
# We can also compare models with and without fixed edf
compareML(mld_gamm_pike_day_k100, mld_gamm_pike_day_k100.fx)    # The fixed model is better

# Model summary
summary.gam(mld_gamm_pike_day_k100.fx)

# Extract R^2 
summary(mld_gamm_pike_day_k100.fx)$r.sq   
[1] 0.88        # R^2 has increased by ~15%/20% with respect to 100-k-non-fx model and default (10k)-non-fx model, respectively

# Check overdispersion (phi)
sum(resid(mld_gamm_pike_day_k100.fx, type = "pearson")^2) / df.residual(mld_gamm_pike_day_k100.fx)
[1] 1.13283    # phi has decreased more than the unit (phi < 1.5)

# Check residuals
qq.gam(mld_gamm_pike_day_k100.fx)             # Much better residuals than previous models
gam.check(mld_gamm_pike_day_k100.fx)          # The k' index seems quite good for all terms

# Plot non-parametric predictors and random smooth effects
par(mfrow = c(2,4))
plot(mld_gamm_pike_day_k100.fx, select = 1, shade = TRUE, scale = 0, seWithMean = TRUE)  # seasonal depth effect
plot(mld_gamm_pike_day_k100.fx, select = 2, shade = TRUE, scale = 0, seWithMean = TRUE)  # amplitude effect
plot(mld_gamm_pike_day_k100.fx, select = 3, shade = TRUE, scale = 0, seWithMean = TRUE)  # mean_gradient effect - Note the linear relationship with det_depth
plot(mld_gamm_pike_day_k100.fx, select = 4, shade = TRUE, scale = 0, seWithMean = TRUE)  # slope of seasonal_depth for each fish
plot(mld_gamm_pike_day_k100.fx, select = 5, shade = TRUE, scale = 0, seWithMean = TRUE)  # slope of amplitude for each fish
plot(mld_gamm_pike_day_k100.fx, select = 6, shade = TRUE, scale = 0, seWithMean = TRUE)  # slope of mean_gradient for each fish
plot(mld_gamm_pike_day_k100.fx, select = 6, shade = TRUE, scale = 0, seWithMean = TRUE)  # time effect
plot(mld_gamm_pike_day_k100.fx, select = 6, shade = TRUE, scale = 0, seWithMean = TRUE)  # slope of time for each fish
...
AIC(mld_gamm_pike_day_k100.fx, mld_gamm_pike_day_final)
                                    df      AIC
mld_gamm_pike_day_k100.fx     2281.943 213072.3
mld_gamm_pike_day_final       2280.522 213072.6     # There are no differences between models but... 

deviance(mld_gamm_pike_day_k100.fx)
[1] 269419                  
